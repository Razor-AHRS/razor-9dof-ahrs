...
#define OUTPUT__MODE_ANGLES_SENSORS_RAW 5
#define OUTPUT__MODE_ANGLES_SENSORS_CALIB 6
...
void loop()
{
      ...
      else if (command == 'o') // Set _o_utput mode
      {
        ...
        else if (output_param == 'm') // Output angles and sensor values.
        {
          ...
          if (values_param == 'r')  // Output _r_aw sensor values
            output_mode = OUTPUT__MODE_ANGLES_SENSORS_RAW;
          else if (values_param == 'c')  // Output _c_alibrated sensor values
            output_mode = OUTPUT__MODE_ANGLES_SENSORS_CALIB;
        }
  ...
  if((millis() - timestamp) >= OUTPUT__DATA_INTERVAL)
  {
    ...
    else if (output_mode == OUTPUT__MODE_ANGLES_SENSORS_CALIB)
    {
      // Apply sensor calibration
      compensate_sensor_errors();
    ...
    else if (output_mode == OUTPUT__MODE_ANGLES_SENSORS_RAW)
    {
      // Run DCM algorithm
      Compass_Heading(); // Calculate magnetic heading
      Matrix_update();
